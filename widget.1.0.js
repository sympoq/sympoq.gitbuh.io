...
if (window.jQuery === undefined || window.jQuery.fn.jquery !== '3.6.0') 
{
	...
	script_tag.setAttribute("sync", true);
...
function loadModalJS(FILE_URL, async = false) {
...
{ 
    ...
    { 
        ...
        var icon = widget[1];
        var color = widget[2];
        var position = widget[3];

        var buttonContent = '<div class="s--button-container" style="bottom:var('+ position +')">'
		+ '<a rel="nofollow" href="https://' + site + '.sympoq.com/app/f?p=widget:view" onclick="$(this).modal({width:380, height:590}).open(); return false;">'
		...
		$(document.body).append(buttonContent);
    });
}
